...
async function main(): Promise<void> {
  ...
  await page.waitForTimeout(6000); // 等 SPA 渲染（豆包渲染较慢）
  ...
  const probe: ProbeResult = await page.evaluate((host: string) => {
    ...
    document.querySelectorAll('button, [role="button"], div[class*="btn"], div[class*="button"], div[class*="send"], div[class*="submit"]').forEach((el, i) => {
    ...
    // 搜索含关键文字的元素（豆包 DOM 混淆 class 不稳，用文字定位最稳）
    // 豆包联网搜索开关可能叫"联网搜索"/"搜索"/"联网"等，需广泛探测
    const keywords = ['联网搜索', '联网', '搜索', '搜索增强', '深度搜索', '智能搜索', 'send', '发送', '提问', '发消息'];
    ...
    document.querySelectorAll('[class*="message"], [class*="chat"], [class*="answer"], [class*="response"], [class*="conversation"], [class*="markdown"], [class*="bubble"]').forEach((el, i) => {
      if (i > 20) return;
  ...
  // 5. 保存页面 HTML（截取前 300KB）
  const html: string = await page.content();
  save('probe-page.html', html.slice(0, 300000));
  ...
  log(`Search text hits: ${JSON.stringify(probe.searchTexts.map(s => ({ kw: s.keyword, tag: s.tag, cls: s.className, text: s.text, pCls: s.parentClass })))}`);
