...
export class DiagnosisConfigurationService {
  ...
  async list(brandId: number) { const brand = await this.brand(brandId); let saved = await this.questions.find({ where: { brandId }, order: { ordr: 'ASC', id: 'ASC' } }); if (!saved.length && brand.questions?.length) saved = await this.questions.save(brand.questions.map((question, ordr) => this.questions.create({ brandId, question, group: '推荐', market: 'cn', brandProbe: false, ordr }))); const items = saved.map((item) => ({ id: item.id, text: item.question, group: item.group, market: item.market, brandProbe: item.brandProbe })); return { questions: items, prompt: brand.questionsPrompt ?? buildBrandQuestionPrompt(brand), sitemapUrlLimit: brand.sitemapUrlLimit ?? defaultSitemapUrlLimit }; }
  ...
  async generate(brandId: number, extraPrompt?: string) {
    ...
    const questions = parseGeneratedQuestions(await this.complete(model, prompt), 10);
    if (questions.length !== 10) throw new BadRequestException(`默认模型仅返回 ${questions.length} 个有效问题，需要严格返回 10 个问题。`);
    return { questions };
  }
