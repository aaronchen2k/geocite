...
export class BrandsService {
  ...
  private async assertUniqueCode(code: string, ignoredId?: number) { const builder = this.repository.createQueryBuilder('brand').where('brand.code = :code', { code }); if (ignoredId !== undefined) builder.andWhere('brand.id != :ignoredId', { ignoredId }); if (await builder.getExists()) throw new ConflictException('Brand code 已存在'); }
}
