...
export class EnginesService {
  ...
  async create(dto: CreateEngineDto) { const item = await this.save(this.repository.create({ ...dto, code: dto.code.trim(), homepage: dto.homepage?.trim() || null, apiKey: dto.apiKey?.trim() || null, modelName: dto.modelName?.trim() || null, baseUrl: dto.baseUrl?.trim() || null })); item.ordr = item.id * 100; return this.toResponse(await this.save(item)); }
  async update(id: number, dto: UpdateEngineDto) { const item = await this.findOne(id); const { apiKey, modelName, baseUrl, homepage, ...rest } = dto; Object.assign(item, rest, dto.code ? { code: dto.code.trim() } : {}); if (apiKey !== undefined) item.apiKey = apiKey.trim() || null; if (modelName !== undefined) item.modelName = modelName.trim() || null; if (baseUrl !== undefined) item.baseUrl = baseUrl.trim() || null; if (homepage !== undefined) item.homepage = homepage.trim() || null; return this.toResponse(await this.save(item)); }
