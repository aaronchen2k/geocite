...
### Task 1.5: Playwright 网页复核开关

**Files:**
- Modify: `server/src/modules/brands/brand.entity.ts`
- Modify: `server/src/modules/execution-diagnosis/diagnosis-configuration.dto.ts`
- Modify: `server/src/modules/execution-diagnosis/diagnosis-configuration.service.ts`
- Modify: `ui/src/components/configuration/basic-configuration-page.tsx`
- Test: `server/src/modules/execution-diagnosis/diagnosis-configuration.service.spec.ts`
- Test: `ui/tests/home.spec.ts`

**Interfaces:**
- 配置响应与保存 DTO 增加 `playwrightWebReviewEnabled: boolean`，默认 `true`。
- 后续执行快照读取该值；关闭时 API 采样照常运行，网页复核步骤标为 `skipped` 且原因 `playwright-web-review-disabled`。

- [x] **Step 1: 写失败测试并确认红灯**

```ts
expect(await service.list(brandId)).toMatchObject({playwrightWebReviewEnabled: true});
await page.getByRole('checkbox', {name: '使用 Playwright 网页复核'}).uncheck();
await page.getByRole('button', {name: '保存'}).click();
expect(saved.playwrightWebReviewEnabled).toBe(false);
```

Run: `pnpm --dir server test -- diagnosis-configuration.service.spec.ts && pnpm --dir ui exec playwright test tests/home.spec.ts --grep "Playwright 网页复核"`

Expected: FAIL，当前响应和页面没有该开关。

- [x] **Step 2: 实现默认开启的配置与推荐文案**

实体列默认为 `true`；配置页面在“诊断采样范围”中增加受控 Switch，标题“使用 Playwright 网页复核”，说明“推荐开启：以网页端真实用户环境抽样校正 API 粗扫结果。关闭后本次仅保留 API 参考结果。”开关和现有配置原子保存。

- [x] **Step 3: 验证并提交**

Run: `pnpm --dir server test -- diagnosis-configuration.service.spec.ts && pnpm --dir ui exec tsc --noEmit && pnpm --dir ui exec playwright test tests/home.spec.ts --grep "Playwright 网页复核"`

Expected: PASS。

```bash
git add server/src/modules/brands/brand.entity.ts server/src/modules/execution-diagnosis/diagnosis-configuration.dto.ts server/src/modules/execution-diagnosis/diagnosis-configuration.service.ts ui/src/components/configuration/basic-configuration-page.tsx server/src/modules/execution-diagnosis/diagnosis-configuration.service.spec.ts ui/tests/home.spec.ts
git commit -m "feat: configure playwright web review"
```
...
- [x] **Step 3: 实现 Profile、launchId 与三态健康检查**
...
- [ ] **Step 2: 手工 Electron 验收（需在客户本机 Chrome 环境执行）**
