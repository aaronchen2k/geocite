...
export function ExecutionDiagnosisPage(): React.JSX.Element {
  ...
  return <section className="pb-8">
    <header className="mb-6 flex flex-col gap-4 border-b border-[var(--border)] lg:flex-row lg:items-start lg:justify-between"><div><div className="mb-2 flex items-center gap-2"><h1 className="text-[22px] font-semibold">{t('title')}</h1><span className="rounded-full bg-[var(--muted)] px-2 py-0.5 text-xs text-[var(--muted-foreground)]">{t('sampleData')}</span></div><p className="text-sm leading-6 text-[var(--muted-foreground)]">{t('description')}</p></div><div className="shrink-0 text-sm text-[var(--muted-foreground)]">{t('lastSuccess')}</div></header>
