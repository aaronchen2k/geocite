...
type WebReviewSummary = {apiTotal: number; minimumTarget: number; mandatoryCore: number; mandatoryMentioned: number; randomUnmentioned: number; minimumFill: number; succeeded: number; excludedByReason: Record<string, number>};
...
function WebReviewEvidence({summary, evidenceBasis}: {summary?: WebReviewSummary; evidenceBasis?: EvidenceBasis}): React.JSX.Element | null {
  ...
  return <section className="mt-5 rounded-lg border border-[var(--border)] bg-[var(--card)] p-4"><div className="flex flex-wrap items-baseline justify-between gap-2"><div><h2 className="font-semibold">网页端真实用户环境复核</h2><p className="mt-1 text-sm text-[var(--muted-foreground)]">{corrected ? '以网页复核校正' : '仅 API 参考，未经过网页复核校正，不输出校正结论'}</p></div><span className="text-sm text-[var(--muted-foreground)]">成功复核 {summary.succeeded} 条</span></div><dl className="mt-4 grid gap-x-6 gap-y-3 text-sm sm:grid-cols-2 xl:grid-cols-4"><div><dt className="text-[var(--muted-foreground)]">API 采样</dt><dd className="mt-1 font-medium">{summary.apiTotal}</dd></div><div><dt className="text-[var(--muted-foreground)]">最低目标</dt><dd className="mt-1 font-medium">{summary.minimumTarget}</dd></div><div><dt className="text-[var(--muted-foreground)]">强制复核</dt><dd className="mt-1 font-medium">核心 {summary.mandatoryCore} · 已提及 {summary.mandatoryMentioned}</dd></div><div><dt className="text-[var(--muted-foreground)]">随机未提及</dt><dd className="mt-1 font-medium">{summary.randomUnmentioned}</dd></div><div><dt className="text-[var(--muted-foreground)]">最低目标补足</dt><dd className="mt-1 font-medium">{summary.minimumFill}</dd></div><div><dt className="text-[var(--muted-foreground)]">排除原因</dt><dd className="mt-1 font-medium">{excluded.length ? excluded.map(([reason, count]) => `${reason} ${count}`).join(' · ') : '—'}</dd></div></dl>{corrected && <p className="mt-5 border-t border-[var(--border)] pt-4 text-sm leading-6 text-[var(--muted-foreground)]">本次共采集 {summary.apiTotal} 条目标问句，全部通过官方 API 批量预扫描；其中 {summary.succeeded} 条关键样本经过网页端真实用户环境复核，最终指标以复核样本校正得出；其余 API 扫描结果仅供参考。</p>}</section>;
}
