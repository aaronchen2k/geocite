...
export function BasicConfigurationPage(): React.JSX.Element {
  ...
  return <section className="max-w-3xl pb-8"><header className="mb-6 border-b border-[var(--border)] pb-4"><h1 className="mb-2 text-[22px] font-semibold">{t('title')}</h1><p className="text-sm leading-6 text-[var(--muted-foreground)]">{t('description')}</p></header>{error && <p role="alert" className="mb-4 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p>}{saved && <p className="mb-4 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-700">{t('saved')}</p>}<div className="grid gap-5"><section className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4"><h2 className="font-semibold">{t('brandSection')}</h2><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="grid gap-1.5 text-sm"><span>{t('name')}</span><input className="h-9 rounded-md border border-[var(--border)] bg-[var(--card)] px-3" value={brand?.name ?? ''} onChange={(event) => setBrand((current) => current ? {...current, name: event.target.value} : current)} /></label><label className="grid gap-1.5 text-sm"><span>{t('website')}</span><input className="h-9 rounded-md border border-[var(--border)] bg-[var(--card)] px-3" value={brand?.website ?? ''} onChange={(event) => setBrand((current) => current ? {...current, website: event.target.value || null} : current)} /></label><label className="grid gap-1.5 text-sm"><span>{t('industry')}</span><input className="h-9 rounded-md border border-[var(--border)] bg-[var(--card)] px-3" value={brand?.industry ?? ''} onChange={(event) => setBrand((current) => current ? {...current, industry: event.target.value || null} : current)} /></label><label className="grid gap-1.5 text-sm sm:col-span-2"><span>{t('descriptionField')}</span><textarea className="min-h-24 rounded-md border border-[var(--border)] bg-[var(--card)] p-3" value={brand?.description ?? ''} onChange={(event) => setBrand((current) => current ? {...current, description: event.target.value || null} : current)} /></label></div></section><section className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4"><h2 className="font-semibold">{t('crawlSection')}</h2><label className="mt-4 grid max-w-md gap-1.5 text-sm"><span>{t('sitemapUrlLimit')}</span><input type="number" min={1} max={100} className="h-9 rounded-md border border-[var(--border)] bg-[var(--card)] px-3" value={configuration?.sitemapUrlLimit ?? ''} onChange={(event) => setConfiguration((current) => current ? {...current, sitemapUrlLimit: Number(event.target.value)} : current)} /><span className="text-xs text-[var(--muted-foreground)]">{t('sitemapUrlLimitDescription')}</span></label></section></div><div className="mt-5 flex justify-end"><Button disabled={!brand || !configuration || saving} onClick={() => void save()}>{saving ? t('saving') : t('save')}</Button></div></section>;
}
