...
export function ResourceManagementPage({ config }: { config: ResourceConfig }): React.JSX.Element {
  ...
  const submit = async (event: FormEvent) => {
    ...
    config.fields.forEach((field) => { const value = values[field.key]; if (field.type === 'checkbox') payload[field.key] = Boolean(value); else if (value !== '') payload[field.key] = field.type === 'number' ? Number(value) : String(value).trim(); });
    try {
