...
const PAGE_SIZE = 20;
export type ResourceItem = Record<string, unknown> & { id: number; name?: string; disabled?: boolean };
...
type Column = { key: string; label: string; sortable?: boolean; render?: (item: Item) => ReactNode };
const selectedValues = (values: Record<string, string | boolean | string[]>, key: string): string[] => Array.isArray(values[key]) ? values[key] : [];

export type ResourceConfig = { endpoint: string; title: string; singular: string; description: string; fields: Field[]; filters: Filter[]; columns: Column[]; actionColumnWidth?: number; renderActions?: (item: Item, onChanged: () => Promise<void>) => ReactNode };
...
export function ResourceManagementPage({ config }: { config: ResourceConfig }): React.JSX.Element {
  ...
  return <section>
    ...
    <section className="overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--card)]">
      <div className="scroll-area overflow-x-auto"><table className="w-full min-w-[720px] text-sm"><thead className="border-b border-[var(--border)] bg-[var(--muted)] text-left text-[var(--muted-foreground)]"><tr>{config.columns.map((column) => <th key={column.key} className="px-3 py-3 font-medium">{column.sortable ? <button className="inline-flex items-center gap-1" onClick={() => toggleSort(column.key)}>{column.label}{sortBy === column.key ? sortOrder === 'ASC' ? <ArrowUp className="size-3.5" /> : <ArrowDown className="size-3.5" /> : null}</button> : column.label}</th>)}<th className="w-40 px-3 py-3 font-medium" style={config.actionColumnWidth ? { width: `${config.actionColumnWidth}px` } : undefined}>{t('actions')}</th></tr></thead><tbody>{items.map((item) => <tr key={item.id} className="border-b border-[var(--border)] last:border-0"><td className="px-3 py-3 font-medium">{item.name}</td>{config.columns.slice(1).map((column) => <td key={column.key} className="px-3 py-3">{column.render ? column.render(item) : item[column.key] === null || item[column.key] === undefined || item[column.key] === '' ? t('none') : String(item[column.key])}</td>)}<td className="w-40 px-3 py-3" style={config.actionColumnWidth ? { width: `${config.actionColumnWidth}px` } : undefined}><div className="flex flex-wrap gap-1">{config.renderActions?.(item, load)}<Button size="icon" variant="ghost" aria-label={t('edit', {name: item.name ?? item.id})} onClick={() => openEdit(item)}><Pencil className="size-4" /></Button><Button size="icon" variant="ghost" className="text-[#b42318]" aria-label={t('delete', {name: item.name ?? item.id})} onClick={() => void remove(item)}><Trash2 className="size-4" /></Button></div></td></tr>)}{!loading && !items.length ? <tr><td colSpan={config.columns.length + 1} className="px-3 py-12 text-center text-[var(--muted-foreground)]">{t('empty', {resource: config.singular})}</td></tr> : null}</tbody></table></div>
