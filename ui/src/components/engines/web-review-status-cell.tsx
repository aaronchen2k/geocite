...
type Engine = {id: number; webReview?: Partial<WebReviewStatus> | null};

const labels: Record<Availability, string> = {unknown: 'unknown', unavailable: 'unavailable', pending_login: 'pendingLogin', ready: 'ready'};

export function normalizeWebReviewAvailability(value: unknown): Availability {
  return typeof value === 'string' && Object.hasOwn(labels, value) ? value as Availability : 'unknown';
}

export function WebReviewStatusCell({engine}: {engine: Engine}): React.JSX.Element {
  const t = useTranslations('Management.webReview');
  const webReview = engine.webReview;
  const status = {availability: normalizeWebReviewAvailability(webReview?.availability), lastCheckedAt: webReview?.lastCheckedAt ?? null, lastFailureReason: webReview?.lastFailureReason ?? null, lastReadyAt: webReview?.lastReadyAt ?? null};
